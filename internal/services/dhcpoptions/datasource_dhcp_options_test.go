		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					provider.TestCheckTypeSetElemNestedAttrsWithPair("data.numspot_dhcp_options.testdata", "items.*", map[string]string{
						"id":          provider.PAIR_PREFIX + "numspot_dhcp_options.test.id",
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					provider.TestCheckTypeSetElemNestedAttrsWithPair("data.numspot_dhcp_options.testdata", "items.*", map[string]string{
						"id":          provider.PAIR_PREFIX + "numspot_dhcp_options.obj1.id",
						"domain_name": domainName1,
					}),
					provider.TestCheckTypeSetElemNestedAttrsWithPair("data.numspot_dhcp_options.testdata", "items.*", map[string]string{
						"id":          provider.PAIR_PREFIX + "numspot_dhcp_options.obj2.id",
