...
func TestAccVpcResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{ // Create testing
				Config: testVpcConfig(provider.BASE_SUFFIX, basePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					createChecks,
					getDependencyChecksVpc(provider.BASE_SUFFIX),
				)...),
			},
			// ImportState testing
			{
			...
			{
				Config: testVpcConfig(provider.BASE_SUFFIX, updatePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks,
					getDependencyChecksVpc(provider.BASE_SUFFIX),
				)...),
			...
			{
				Config: testVpcConfig(provider.BASE_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksVpc(provider.BASE_SUFFIX),
				)...),
			...
			{
				Config: testVpcConfig(provider.NEW_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksVpc(provider.NEW_SUFFIX),
				)...),
			},
