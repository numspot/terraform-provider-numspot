...
func TestAccSecurityGroupResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{ // Create testing
				Config: testSecurityGroupConfig(provider.BASE_SUFFIX, basePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					createChecks,
					getDependencyChecksSecurityGroup(provider.BASE_SUFFIX),
				)...),
			},
			// ImportState testing
			{
			...
			{
				Config: testSecurityGroupConfig(provider.BASE_SUFFIX, updatePlanValues_1),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks_1,
					getDependencyChecksSecurityGroup(provider.BASE_SUFFIX),
				)...),
			...
			// Update testing Without Replace (if needed)
			{
				Config: testSecurityGroupConfig(provider.BASE_SUFFIX, updatePlanValues_2),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks_2,
					getDependencyChecksSecurityGroup(provider.BASE_SUFFIX),
				)...),
			...
			{
				Config: testSecurityGroupConfig(provider.BASE_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksSecurityGroup(provider.BASE_SUFFIX),
				)...),
...
}`, data.name, data.description, inboundRules, outboundRules, data.tagKey, data.tagValue)
}
