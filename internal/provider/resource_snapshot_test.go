...
func getDependencyChecksSnapshot_FromSnapshot(dependenciesPrefix string) []resource.TestCheckFunc {
	return []resource.TestCheckFunc{
		resource.TestCheckResourceAttrPair("numspot_snapshot.test", "source_snapshot_id", "numspot_snapshot.test"+dependenciesPrefix, "id"),
	}
}
...
func TestAccSnapshotResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{ // Create testing
				Config: testSnapshotConfig_FromVolume(utils_acctest.BASE_SUFFIX, basePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					createChecks,
					getDependencyChecksSnapshot_FromVolume(utils_acctest.BASE_SUFFIX),
				)...),
			},
			// ImportState testing
			{
			...
			{
				Config: testSnapshotConfig_FromVolume(utils_acctest.BASE_SUFFIX, updatePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks,
					getDependencyChecksSnapshot_FromVolume(utils_acctest.BASE_SUFFIX),
				)...),
			...
			{
				Config: testSnapshotConfig_FromVolume(utils_acctest.BASE_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksSnapshot_FromVolume(utils_acctest.BASE_SUFFIX),
				)...),
			...
			{
				Config: testSnapshotConfig_FromVolume(utils_acctest.NEW_SUFFIX, updatePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks,
					getDependencyChecksSnapshot_FromVolume(utils_acctest.NEW_SUFFIX),
				)...),
			},
			...
			{
				Config: testSnapshotConfig_FromVolume(utils_acctest.NEW_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksSnapshot_FromVolume(utils_acctest.NEW_SUFFIX),
				)...),
			...
			{ // Create testing
				Config: testSnapshotConfig_FromSnapshot(utils_acctest.BASE_SUFFIX, basePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					createChecks,
					getDependencyChecksSnapshot_FromSnapshot(utils_acctest.BASE_SUFFIX),
				)...),
			},
			// ImportState testing
			{
			...
			// Update testing Without Replace (if needed)
			{
				Config: testSnapshotConfig_FromSnapshot(utils_acctest.BASE_SUFFIX, updatePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks,
					getDependencyChecksSnapshot_FromSnapshot(utils_acctest.BASE_SUFFIX),
				)...),
			...
			// Update testing With Replace (if needed)
			{
				Config: testSnapshotConfig_FromSnapshot(utils_acctest.BASE_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksSnapshot_FromSnapshot(utils_acctest.BASE_SUFFIX),
				)...),
			...
			// Note : due to Numspot APIs architecture, this use case will not work in most cases. Nothing can be done on provider side to fix this
			{
				Config: testSnapshotConfig_FromSnapshot(utils_acctest.NEW_SUFFIX, updatePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks,
					getDependencyChecksSnapshot_FromSnapshot(utils_acctest.NEW_SUFFIX),
				)...),
			},
			// Update testing With Replace of dependency resource and with Replace of the resource (if needed)
			// This test is useful to check wether or not the deletion of the dependencies and then the deletion of the main resource works properly
			{
				Config: testSnapshotConfig_FromSnapshot(utils_acctest.NEW_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksSnapshot_FromSnapshot(utils_acctest.NEW_SUFFIX),
				)...),
...
resource "numspot_snapshot" "test" {
  source_snapshot_id = numspot_snapshot.test%[1]s.id
