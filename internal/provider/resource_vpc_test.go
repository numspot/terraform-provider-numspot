...
func TestAccVpcResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{ // Create testing
				Config: testVpcConfig(utils_acctest.BASE_SUFFIX, basePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					createChecks,
					getDependencyChecksVpc(utils_acctest.BASE_SUFFIX),
				)...),
			},
			// ImportState testing
			{
			...
			{
				Config: testVpcConfig(utils_acctest.BASE_SUFFIX, updatePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks,
					getDependencyChecksVpc(utils_acctest.BASE_SUFFIX),
				)...),
			...
			{
				Config: testVpcConfig(utils_acctest.BASE_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksVpc(utils_acctest.BASE_SUFFIX),
				)...),
			...
			{
				Config: testVpcConfig(utils_acctest.NEW_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksVpc(utils_acctest.NEW_SUFFIX),
				)...),
			},
...
// <== If resource has optional dependencies ==>
func testVpcConfig_DeletedDependencies(data StepDataVpc) string {
	return fmt.Sprintf(`
resource "numspot_vpc" "test" {
  ip_range = %[1]q
  tenancy  = %[2]q
...
}`, data.ipRange, data.tenancy, data.tagKey, data.tagValue)
}
