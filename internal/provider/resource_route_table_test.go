...
func getDependencyChecksRouteTable(dependenciesPrefix string) []resource.TestCheckFunc {
	return []resource.TestCheckFunc{
		...
		utils_acctest.TestCheckTypeSetElemNestedAttrsWithPair("numspot_route_table.test", "routes.*", map[string]string{ // If field is a list of objects (containing id and/or other fields)
			"gateway_id": fmt.Sprintf(utils_acctest.PAIR_PREFIX+"numspot_internet_gateway.test%[1]s.id", dependenciesPrefix),
		}),
	}
}
...
func TestAccRouteTableResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{ // Create testing
				Config: testRouteTableConfig(utils_acctest.BASE_SUFFIX, basePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					createChecks,
					getDependencyChecksRouteTable(utils_acctest.BASE_SUFFIX),
				)...),
			},
			// ImportState testing
			{
			...
			{
				Config: testRouteTableConfig(utils_acctest.BASE_SUFFIX, updatePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks,
					getDependencyChecksRouteTable(utils_acctest.BASE_SUFFIX),
				)...),
			...
			{
				Config: testRouteTableConfig(utils_acctest.NEW_SUFFIX, updatePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks,
					getDependencyChecksRouteTable(utils_acctest.NEW_SUFFIX),
				)...),
			},
			...
			// Note : due to Numspot APIs architecture, this use case will not work in most cases. Nothing can be done on provider side to fix this
			{
				Config: testRouteTableConfig_DeletedDependencies(updatePlanValues),
