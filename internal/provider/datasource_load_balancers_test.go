		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					utils_acctest.TestCheckTypeSetElemNestedAttrsWithPair("data.numspot_load_balancers.testdata", "items.*", map[string]string{
						"id":                                 utils_acctest.PAIR_PREFIX + "numspot_load_balancer.test.id",
						"subnets.0":                          utils_acctest.PAIR_PREFIX + "numspot_subnet.subnet.id",
						"name":                               name,
