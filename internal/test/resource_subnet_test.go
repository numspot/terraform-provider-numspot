...
func TestAccSubnetResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{ // Create testing
				Config: testSubnetConfig(acctest.BASE_SUFFIX, basePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					createChecks,
					getDependencyChecksSubnet(acctest.BASE_SUFFIX),
				)...),
			},
			// ImportState testing
			{
			...
			{
				Config: testSubnetConfig(acctest.BASE_SUFFIX, updatePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks,
					getDependencyChecksSubnet(acctest.BASE_SUFFIX),
				)...),
			...
			{
				Config: testSubnetConfig(acctest.BASE_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksSubnet(acctest.BASE_SUFFIX),
				)...),
			...
			{
				Config: testSubnetConfig(acctest.NEW_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksSubnet(acctest.NEW_SUFFIX),
				)...),
			},
		},
	})
