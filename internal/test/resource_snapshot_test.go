...
func TestAccSnapshotResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{ // Create testing
				Config: testSnapshotConfig_FromVolume(acctest.BASE_SUFFIX, basePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					createChecks,
					getDependencyChecksSnapshot_FromVolume(acctest.BASE_SUFFIX),
				)...),
			},
			// ImportState testing
			{
			...
			{
				Config: testSnapshotConfig_FromVolume(acctest.BASE_SUFFIX, updatePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks,
					getDependencyChecksSnapshot_FromVolume(acctest.BASE_SUFFIX),
				)...),
			...
			{
				Config: testSnapshotConfig_FromVolume(acctest.BASE_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksSnapshot_FromVolume(acctest.BASE_SUFFIX),
				)...),
			...
			{
				Config: testSnapshotConfig_FromVolume(acctest.NEW_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksSnapshot_FromVolume(acctest.NEW_SUFFIX),
				)...),
			},

			//////// Test snapshot created from another Snapshot
			{ // Create testing
				Config: testSnapshotConfig_FromSnapshot(acctest.BASE_SUFFIX, basePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					createChecks,
					getDependencyChecksSnapshot_FromSnapshot(acctest.BASE_SUFFIX),
				)...),
			},
			// ImportState testing
			{
			...
			// Update testing Without Replace (if needed)
			{
				Config: testSnapshotConfig_FromSnapshot(acctest.BASE_SUFFIX, updatePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks,
					getDependencyChecksSnapshot_FromSnapshot(acctest.BASE_SUFFIX),
				)...),
			...
			// Update testing With Replace (if needed)
			{
				Config: testSnapshotConfig_FromSnapshot(acctest.BASE_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksSnapshot_FromSnapshot(acctest.BASE_SUFFIX),
				)...),
			...
			// This test is useful to check wether or not the deletion of the dependencies and then the deletion of the main resource works properly
			{
				Config: testSnapshotConfig_FromSnapshot(acctest.NEW_SUFFIX, replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksSnapshot_FromSnapshot(acctest.NEW_SUFFIX),
				)...),
			},
		},
	})
