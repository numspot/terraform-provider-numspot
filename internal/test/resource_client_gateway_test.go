...
func TestAccClientGatewayResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{ // Create testing
				Config: testClientGatewayConfig(basePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					createChecks,
					getDependencyChecksClientGateway(acctest.BASE_SUFFIX),
				)...),
			},
			// ImportState testing
			{
			...
			{
				Config: testClientGatewayConfig(updatePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					updateChecks,
					getDependencyChecksClientGateway(acctest.BASE_SUFFIX),
				)...),
			...
			{
				Config: testClientGatewayConfig(replacePlanValues),
				Check: resource.ComposeAggregateTestCheckFunc(slices.Concat(
					replaceChecks,
					getDependencyChecksClientGateway(acctest.BASE_SUFFIX),
				)...),
...
func testClientGatewayConfig(data StepDataClientGateway) string {
	return fmt.Sprintf(`
